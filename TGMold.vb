...
Public Class TGMold
    Private Sub TGMold_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadTGPanels()
        DTP_TGS.Value = DateTime.Now
        DGV_M1_TG.AllowDrop = True
        DGV_MB_TG.AllowDrop = True
        DGV_TG_M5.AllowDrop = True
        Panel_TG.Visible = False
    End Sub
    Private Sub LoadTGPanels()
        DGV_35TG.Rows.Clear()
        ...
        Try
            Using connection As New SqlConnection(ConnectionString)
                ...
                Dim query As String = "SELECT [ID], [Panel], [QTY], [ASGM Date] " &
                                  "FROM [Jobs] " &
                                  "WHERE [Panel Identifier] = 'T&G' AND [Status] IS NULL"
                ...
                Using command As New SqlCommand(query, connection)
                    Using reader As SqlDataReader = command.ExecuteReader()
                        While reader.Read()
                            ...
                            DGV_35TG.Rows.Add(panelID, panel, qty, DateSGM)
                        End While
                    End Using
                End Using

                If DGV_35TG.Columns.Count > 0 Then
                    DGV_35TG.Columns(0).HeaderText = "ID"
                    DGV_35TG.Columns(1).HeaderText = "Panel"
                    DGV_35TG.Columns(2).HeaderText = "Qty"
                    DGV_35TG.Columns(3).HeaderText = "Date"
                End If
            End Using
    ...
    Private Sub DGV_35TG_DragDrop(sender As Object, e As DragEventArgs) Handles DGV_35TG.DragDrop
        If e.Data.GetDataPresent(DataFormats.Text) Then
            ...
            Dim rowToUpdate As DataGridViewRow = FindPanelRow(panel, DGV_M1_TG, DGV_MB_TG, DGV_TG_M5)
            ...
            If rowToUpdate IsNot Nothing Then
                If originalQty > 1 Then
                    ...
                    If panelsQtyForm.ShowDialog() = DialogResult.OK Then
                        ...
                        Else
                            If DGV_M1_TG.Rows.Contains(rowToUpdate) Then
                                DGV_M1_TG.Rows.Remove(rowToUpdate) ' Eliminar si la cantidad restante es 0
                            ElseIf DGV_MB_TG.Rows.Contains(rowToUpdate) Then
                                DGV_MB_TG.Rows.Remove(rowToUpdate) ' Eliminar si la cantidad restante es 0
                            ElseIf DGV_TG_M5.Rows.Contains(rowToUpdate) Then
                                DGV_TG_M5.Rows.Remove(rowToUpdate)
                            End If
                ...
                Else
                    ...
                    DGV_35TG.Rows.Add(newRow)
                    RemoveRowFromSource(panel, DGV_M1_TG, DGV_MB_TG, DGV_TG_M5)
                End If
            Else
    ...
    Private Function FindPanelRow(panel As String, ParamArray gridViews() As DataGridView) As DataGridViewRow
        ...
        Return Nothing
    End Function
    Private Sub DGV_35TG_MouseDown(sender As Object, e As MouseEventArgs) Handles DGV_35TG.MouseDown
        If e.Button = MouseButtons.Left Then
            Dim hitTestInfo As DataGridView.HitTestInfo = DGV_35TG.HitTest(e.X, e.Y)
            If hitTestInfo.RowIndex >= 0 Then
                Dim panel As String = DGV_35TG.Rows(hitTestInfo.RowIndex).Cells(1).Value.ToString()
                ' Iniciar el arrastre con el panel como dato
                DGV_35TG.DoDragDrop(panel, DragDropEffects.Move)
            End If
        End If
    End Sub
    Private Sub DGV_M1_TG_DragEnter(sender As Object, e As DragEventArgs) Handles DGV_M1_TG.DragEnter
        If e.Data.GetDataPresent(DataFormats.Text) Then
        ...
        End If
    End Sub
    Private Sub DGV_M1_TG_DragDrop(sender As Object, e As DragEventArgs) Handles DGV_M1_TG.DragDrop
        If e.Data.GetDataPresent(DataFormats.Text) Then
            Dim panel As String = CType(e.Data.GetData(DataFormats.Text), String)
            ...
            For Each row As DataGridViewRow In DGV_35TG.Rows
            ...
            If rowToUpdate IsNot Nothing Then
                ...
                Dim newRow As DataGridViewRow = New DataGridViewRow()
                newRow.CreateCells(DGV_M1_TG)
                newRow.Cells(0).Value = rowToUpdate.Cells(0).Value
                newRow.Cells(1).Value = panel
                newRow.Cells(2).Value = selectedQty
                newRow.Cells(3).Value = DTP_TGS.Value
                DGV_M1_TG.Rows.Add(newRow)

                Dim remainingQty As Integer = originalQty - selectedQty
                If remainingQty > 0 Then
                    rowToUpdate.Cells(2).Value = remainingQty
                Else
                    DGV_35TG.Rows.Remove(rowToUpdate)
                End If
            Else
                MessageBox.Show("No se encontró el panel.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
        End If
    End Sub
    Private Sub DGV_M1_TG_DragOver(sender As Object, e As DragEventArgs) Handles DGV_M1_TG.DragOver
        e.Effect = DragDropEffects.Move
    End Sub
    Private Sub DGV_M1_TG_KeyDown(sender As Object, e As KeyEventArgs) Handles DGV_M1_TG.KeyDown
        If e.KeyCode = Keys.Delete Then
            HandlePanelDeletion(DGV_M1_TG)
        End If
    End Sub
    Private Sub DGV_M1_TG_MouseDown(sender As Object, e As MouseEventArgs) Handles DGV_M1_TG.MouseDown
        Dim hit As DataGridView.HitTestInfo = DGV_M1_TG.HitTest(e.X, e.Y)
        If hit.RowIndex >= 0 AndAlso hit.ColumnIndex >= 0 Then
            Dim cell As DataGridViewCell = DGV_M1_TG.Rows(hit.RowIndex).Cells(hit.ColumnIndex)
            If cell.Selected Then
                Dim dragValue As String = DGV_M1_TG.Rows(hit.RowIndex).Cells(0).Value.ToString()
                DGV_M1_TG.DoDragDrop(dragValue, DragDropEffects.Move)
            End If
        End If
    End Sub
    Private Sub DGV_MB_TG_DragEnter(sender As Object, e As DragEventArgs) Handles DGV_MB_TG.DragEnter
        If e.Data.GetDataPresent(DataFormats.Text) Then
            e.Effect = DragDropEffects.Move
        Else
            e.Effect = DragDropEffects.None
        End If
    End Sub
    Private Sub DGV_MB_TG_W_DragDrop(sender As Object, e As DragEventArgs) Handles DGV_MB_TG.DragDrop
        If e.Data.GetDataPresent(DataFormats.Text) Then
            Dim panel As String = CType(e.Data.GetData(DataFormats.Text), String)
            ...
            If rowToUpdate IsNot Nothing Then
                ...
                Dim newRow As DataGridViewRow = New DataGridViewRow()
                newRow.CreateCells(DGV_MB_TG)
                newRow.Cells(0).Value = rowToUpdate.Cells(0).Value
                newRow.Cells(1).Value = panel
                newRow.Cells(2).Value = selectedQty
                newRow.Cells(3).Value = DTP_TGS.Value
                DGV_MB_TG.Rows.Add(newRow)
        ...
        End If
    End Sub
    Private Sub DGV_MB_TG_MouseDown(sender As Object, e As MouseEventArgs) Handles DGV_MB_TG.MouseDown
        Dim hit As DataGridView.HitTestInfo = DGV_MB_TG.HitTest(e.X, e.Y)
        If hit.RowIndex >= 0 AndAlso hit.ColumnIndex >= 0 Then
            Dim cell As DataGridViewCell = DGV_MB_TG.Rows(hit.RowIndex).Cells(hit.ColumnIndex)
            If cell.Selected Then
                Dim dragValue As String = DGV_MB_TG.Rows(hit.RowIndex).Cells(0).Value.ToString()
                DGV_MB_TG.DoDragDrop(dragValue, DragDropEffects.Move)
            End If
        End If
    End Sub
    Private Sub DGV_MB_TG_KeyDown(sender As Object, e As KeyEventArgs) Handles DGV_MB_TG.KeyDown
        If e.KeyCode = Keys.Delete Then
            HandlePanelDeletion(DGV_MB_TG)
        End If
    End Sub
    Private Sub DGV_MB_TG_DragOver(sender As Object, e As DragEventArgs) Handles DGV_MB_TG.DragOver
        e.Effect = DragDropEffects.Move
    End Sub
    Private Sub M5(sender As Object, e As DragEventArgs) Handles DGV_TG_M5.DragEnter
        If e.Data.GetDataPresent(DataFormats.Text) Then
            e.Effect = DragDropEffects.Move
        Else
            e.Effect = DragDropEffects.None
        End If
    End Sub
    Private Sub DGV_TG_M5_DragDrop(sender As Object, e As DragEventArgs) Handles DGV_TG_M5.DragDrop
        If e.Data.GetDataPresent(DataFormats.Text) Then
            Dim panel As String = CType(e.Data.GetData(DataFormats.Text), String)
            ...
            If rowToUpdate IsNot Nothing Then
                ...
                Dim newRow As DataGridViewRow = New DataGridViewRow()
                newRow.CreateCells(DGV_TG_M5)
                newRow.Cells(0).Value = rowToUpdate.Cells(0).Value
                newRow.Cells(1).Value = panel
                newRow.Cells(2).Value = selectedQty
                newRow.Cells(3).Value = DTP_TGS.Value
                DGV_TG_M5.Rows.Add(newRow)
        ...
        End If
    End Sub
    Private Sub DGV_TG_M5_DragOver(sender As Object, e As DragEventArgs) Handles DGV_TG_M5.DragOver
        e.Effect = DragDropEffects.Move
    End Sub
    Private Sub DGV_TG_M5_KeyDown(sender As Object, e As KeyEventArgs) Handles DGV_TG_M5.KeyDown
        If e.KeyCode = Keys.Delete Then
            HandlePanelDeletion(DGV_TG_M5)
        End If
    End Sub
    Private Sub DGV_TG_M5_MouseDown(sender As Object, e As MouseEventArgs) Handles DGV_TG_M5.MouseDown
        Dim hit As DataGridView.HitTestInfo = DGV_TG_M5.HitTest(e.X, e.Y)
        If hit.RowIndex >= 0 AndAlso hit.ColumnIndex >= 0 Then
            Dim cell As DataGridViewCell = DGV_TG_M5.Rows(hit.RowIndex).Cells(hit.ColumnIndex)
            If cell.Selected Then
                Dim dragValue As String = DGV_TG_M5.Rows(hit.RowIndex).Cells(0).Value.ToString()
                DGV_TG_M5.DoDragDrop(dragValue, DragDropEffects.Move)
            End If
        End If
    End Sub
    ...
    Private Sub MovePanelBackToDGVWalls(panelID As String, panel As String, qty As Integer)
        ...
        For Each wallRow As DataGridViewRow In DGV_35TG.Rows
        ...
        If Not existsInWalls Then
            Dim newRow As DataGridViewRow = CType(DGV_35TG.Rows(0).Clone(), DataGridViewRow)
    ...
    Private Sub RemoveRowFromSource(panel As String, ParamArray grids() As DataGridView)
        ...
        Next
    End Sub
    Private Sub Btn_Save_TGS_Click(sender As Object, e As EventArgs) Handles Btn_Save_TGS.Click
        SavePanels()
    End Sub
    ...
    Private Sub Btn_Menu_Click(sender As Object, e As EventArgs) Handles Btn_Menu.Click
        ...
        Me.Close()
    End Sub
    Private Sub Lnk_lbl_WA_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles Lnk_lbl_WA.LinkClicked
        Me.Hide()
    ...
    Private Sub Lnk_lbl_Tee_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles Lnk_lbl_Tee.LinkClicked
        ...
        Me.Close()
    End Sub
    Private Sub Print_M1_TG_Click(sender As Object, e As EventArgs) Handles Print_M1_TG.Click
        SavePanels()
        Dim queryAB As String = "SELECT [ASGMT Date], [Panel], [Qty] FROM [Molds] WHERE [Mold] = 'DGV_M1_TG' AND [Estatus] = 'NUEVO' ORDER BY [ASGMT Date] ASC"
        ...
        AddHandler printDoc.PrintPage, Sub(s, ev)
                                           ...
                                           Dim headerText As String = "35's TG"
    ...
    End Sub
    Private Sub Print_MB_Click(sender As Object, e As EventArgs) Handles Print_MB.Click
        SavePanels()
        Dim queryAB As String = "SELECT [ASGMT Date], [Panel], [Qty] FROM [Molds] WHERE [Mold] = 'DGV_MB_TG' AND [Estatus] = 'NUEVO' ORDER BY [ASGMT Date] ASC"
        Dim dtAB As New DataTable()
        ...
        AddHandler printDoc.PrintPage, Sub(s, ev)
                                           ...
                                           ' Encabezados de las columnas
                                           ev.Graphics.DrawString("Date", columnHeaderFont, Brushes.Black, tableStartX, yPos)
                                           ev.Graphics.DrawString("Mold B", columnHeaderFont, Brushes.Black, tableStartX + dateColumnWidth, yPos)
                                           ev.Graphics.DrawString("Qty", columnHeaderFont, Brushes.Black, tableStartX + dateColumnWidth + panelColumnWidth, yPos)
                                           yPos += lineHeight
        ...
        printPreviewDialog.ShowDialog()
    End Sub
    Private Sub Print_M5_Click(sender As Object, e As EventArgs) Handles Print_M5.Click
        SavePanels()
        Dim queryAB As String = "SELECT [ASGMT Date], [Panel], [Qty] FROM [Molds] WHERE [Mold] = 'DGV_TG_M5' AND [Estatus] = 'NUEVO' ORDER BY [ASGMT Date] ASC"
        Dim dtAB As New DataTable()
        ...
        AddHandler printDoc.PrintPage, Sub(s, ev)
                                           ...
                                           ' Encabezados de las columnas
                                           ev.Graphics.DrawString("Date", columnHeaderFont, Brushes.Black, tableStartX, yPos)
                                           ev.Graphics.DrawString("Mold 5", columnHeaderFont, Brushes.Black, tableStartX + dateColumnWidth, yPos)
                                           ev.Graphics.DrawString("Qty", columnHeaderFont, Brushes.Black, tableStartX + dateColumnWidth + panelColumnWidth, yPos)
                                           yPos += lineHeight
        ...
        printPreviewDialog.ShowDialog()
    End Sub
    Private Sub Print_All_TG_Click(sender As Object, e As EventArgs) Handles Print_All_TG.Click
        SavePanels()

        Dim queryAB As String = "SELECT [ASGMT Date], [Mold], [Panel], [Qty] " &
                        "FROM [Molds] " &
                        "WHERE ([Mold] = 'DGV_M1_TG' OR [Mold] = 'DGV_MB_TG' OR [Mold] = 'DGV_TG_M5' " &
                        "AND [Estatus] = 'NUEVO' " &
    ...
    Private Sub FilterPanelsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles FilterPanelsToolStripMenuItem.Click
        Panel_TG.Visible = True
        LoadJobsTG()
    End Sub
    Private Sub LoadJobsTG()
        ...
        Try
            Using connection As New SqlConnection(connectionString)
                ...
                Dim query As String = $"SELECT DISTINCT [Job Number] FROM Jobs WHERE [Panel Identifier] = 'T&G' AND [Status] IS NULL"
                Using command As New SqlCommand(query, connection)
                    Using reader As SqlDataReader = command.ExecuteReader()
                        While reader.Read()
    ...
    Private Sub Txt_Bx_Jobs_TextChanged(sender As Object, e As EventArgs) Handles Txt_Bx_Jobs.TextChanged
        ...
        For i As Integer = 0 To Chckd_lst_TG.Items.Count - 1
            Dim itemText As String = Chckd_lst_TG.Items(i).ToString().ToLower()

            If itemText.Contains(searchJob) Then
                Chckd_lst_TG.SetItemChecked(i, True)
            Else
                Chckd_lst_TG.SetItemChecked(i, False)
            End If
        Next
    End Sub
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        If Chckd_lst_TG.CheckedItems.Count > 0 Then
            Dim JobFilter As New List(Of String)
            Dim index As Integer = 0
            For Each item As String In Chckd_lst_TG.CheckedItems
        ...
        Try
            Using connection As New SqlConnection(connectionString)
                ...
                Using reader As SqlDataReader = command.ExecuteReader()
                    DGV_35TG.Rows.Clear() ' Limpiar filas existentes
                    While reader.Read()
                        Dim panelID As String = reader("ID").ToString()
                        Dim panel As String = reader("Panel").ToString()
                        Dim qty As Integer = Convert.ToInt32(reader("QTY"))
