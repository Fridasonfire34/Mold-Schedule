...
Public Class FCT_Mold
    Private Sub FCT_Mold_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadFCTMPanels()
        DTP_FCTM.Value = DateTime.Now
        DGV_FCTM_Panels.AllowDrop = True
        DGV_FCTM.AllowDrop = True
        Panel_FCTM.Visible = False
    End Sub
    Private Sub LoadFCTMPanels()
        DGV_FCTM_Panels.Rows.Clear()
        ...
        Try
            Using connection As New SqlConnection(ConnectionString)
                ...
                Dim query As String = "SELECT [ID], [Panel], [QTY], [ASGM Date] " &
                      "FROM [Jobs] " &
                      "WHERE ([Panel Identifier] = 'Floor T' OR [Panel Identifier] = 'Ceiling T') " &
                      "AND [Status] IS NULL " &
                      "ORDER BY [Panel Identifier] ASC"
                ...
                Using command As New SqlCommand(query, connection)
                    Using reader As SqlDataReader = command.ExecuteReader()
                        While reader.Read()
                            ...
                            DGV_FCTM_Panels.Rows.Add(panelID, panel, qty, DateSGM)
                        End While
                    End Using
                End Using

                If DGV_FCTM_Panels.Columns.Count > 0 Then
                    DGV_FCTM_Panels.Columns(0).HeaderText = "ID"
                    DGV_FCTM_Panels.Columns(1).HeaderText = "Panel"
                    DGV_FCTM_Panels.Columns(2).HeaderText = "Qty"
                    DGV_FCTM_Panels.Columns(3).HeaderText = "Date"
                End If
            End Using
    ...
    Private Sub DGV_FCTM_Panels_DragDrop(sender As Object, e As DragEventArgs) Handles DGV_FCTM_Panels.DragDrop
        If e.Data.GetDataPresent(DataFormats.Text) Then
            ...
            Dim rowToUpdate As DataGridViewRow = FindPanelRow(panel, DGV_FCTM_Panels)
            ...
            If rowToUpdate IsNot Nothing Then
                If originalQty > 1 Then
                    ...
                    If panelsQtyForm.ShowDialog() = DialogResult.OK Then
                        ...
                        newRow.CreateCells(DGV_FCTM_Panels)
                        ...
                        DGV_FCTM_Panels.Rows.Add(newRow)
                        ...
                        Else
                            If DGV_FCTM.Rows.Contains(rowToUpdate) Then
                                DGV_FCTM.Rows.Remove(rowToUpdate) ' Eliminar si la cantidad restante es 0
                            End If
                ...
                Else
                    ...
                    DGV_FCTM_Panels.Rows.Add(newRow)
                    RemoveRowFromSource(panel, DGV_FCTM)
                End If
            Else
    ...
    Private Function FindPanelRow(panel As String, ParamArray gridViews() As DataGridView) As DataGridViewRow
        ...
        Return Nothing
    End Function
    Private Sub DGV_FCTM_Panels_MouseDown(sender As Object, e As MouseEventArgs) Handles DGV_FCTM_Panels.MouseDown
        If e.Button = MouseButtons.Left Then
            Dim hitTestInfo As DataGridView.HitTestInfo = DGV_FCTM_Panels.HitTest(e.X, e.Y)
            If hitTestInfo.RowIndex >= 0 Then
                Dim panel As String = DGV_FCTM_Panels.Rows(hitTestInfo.RowIndex).Cells(1).Value.ToString()
                ' Iniciar el arrastre con el panel como dato
                DGV_FCTM_Panels.DoDragDrop(panel, DragDropEffects.Move)
            End If
        End If
    End Sub
    Private Sub DGV_FCTM_DragEnter(sender As Object, e As DragEventArgs) Handles DGV_FCTM.DragEnter
        If e.Data.GetDataPresent(DataFormats.Text) Then
        ...
        End If
    End Sub
    Private Sub DGV_FCTM_DragDrop(sender As Object, e As DragEventArgs) Handles DGV_FCTM.DragDrop
        If e.Data.GetDataPresent(DataFormats.Text) Then
            Dim panel As String = CType(e.Data.GetData(DataFormats.Text), String)
            ...
            For Each row As DataGridViewRow In DGV_FCTM_Panels.Rows
            ...
            If rowToUpdate IsNot Nothing Then
                ...
                Dim newRow As DataGridViewRow = New DataGridViewRow()
                newRow.CreateCells(DGV_FCTM)
                newRow.Cells(0).Value = rowToUpdate.Cells(0).Value
                newRow.Cells(1).Value = panel
                newRow.Cells(2).Value = selectedQty
                newRow.Cells(3).Value = DTP_FCTM.Value
                DGV_FCTM.Rows.Add(newRow)

                Dim remainingQty As Integer = originalQty - selectedQty
                If remainingQty > 0 Then
                    rowToUpdate.Cells(2).Value = remainingQty
                Else
                    DGV_FCTM_Panels.Rows.Remove(rowToUpdate)
                End If
            Else
                MessageBox.Show("No se encontró el panel.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
        End If
    End Sub
    Private Sub DGV_FCTM_DragOver(sender As Object, e As DragEventArgs) Handles DGV_FCTM.DragOver
        e.Effect = DragDropEffects.Move
    End Sub
    Private Sub DGV_FCTM_KeyDown(sender As Object, e As KeyEventArgs) Handles DGV_FCTM.KeyDown
        If e.KeyCode = Keys.Delete Then
            HandlePanelDeletion(DGV_FCTM)
        End If
    End Sub
    Private Sub DGV_FCTM_MouseDown(sender As Object, e As MouseEventArgs) Handles DGV_FCTM.MouseDown
        Dim hit As DataGridView.HitTestInfo = DGV_FCTM.HitTest(e.X, e.Y)
        If hit.RowIndex >= 0 AndAlso hit.ColumnIndex >= 0 Then
            Dim cell As DataGridViewCell = DGV_FCTM.Rows(hit.RowIndex).Cells(hit.ColumnIndex)
            If cell.Selected Then
                Dim dragValue As String = DGV_FCTM.Rows(hit.RowIndex).Cells(0).Value.ToString()
                DGV_FCTM.DoDragDrop(dragValue, DragDropEffects.Move)
            End If
        End If
    End Sub
    ...
    Private Sub MovePanelBackToDGVTee(panelID As String, panel As String, qty As Integer)
        ...
        For Each wallRow As DataGridViewRow In DGV_FCTM_Panels.Rows
        ...
        If Not existsInWalls Then
            Dim newRow As DataGridViewRow = CType(DGV_FCTM_Panels.Rows(0).Clone(), DataGridViewRow)
    ...
    Private Sub Lnk_lbl_GTL_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles Lnk_lbl_GTL.LinkClicked
        ...
        Me.Close()
    End Sub
    Private Sub Btn_Save_FCTM_Click(sender As Object, e As EventArgs) Handles Btn_Save_FCTM.Click
        SavePanels()
    End Sub
    ...
    Private Sub Btn_Menu_Click(sender As Object, e As EventArgs) Handles Btn_Menu.Click
        ...
        form1.ShowDialog()
        Me.Close()  ' Cierra el formulario original después de que se cierre el Form1.
    End Sub
    Private Sub PrintMold_Click(sender As Object, e As EventArgs) Handles PrintMold.Click
        SavePanels()
        Dim queryAB As String = "SELECT [ASGMT Date], [Panel], [Qty] FROM [Molds] WHERE [Mold] = 'DGV_FCTM' AND [Estatus] = 'NUEVO' ORDER BY [ASGMT Date] ASC"
    ...
    Private Sub Txt_Bx_Jobs_TextChanged(sender As Object, e As EventArgs) Handles Txt_Bx_Jobs.TextChanged
        ...
        For i As Integer = 0 To Chckd_lst_FCTM.Items.Count - 1
            Dim itemText As String = Chckd_lst_FCTM.Items(i).ToString().ToLower()

            If itemText.Contains(searchJob) Then
                Chckd_lst_FCTM.SetItemChecked(i, True)
            Else
                Chckd_lst_FCTM.SetItemChecked(i, False)
            End If
        Next
    End Sub
    Private Sub Filter_FCTM_Click(sender As Object, e As EventArgs) Handles Filter_FCTM.Click
        Panel_FCTM.Visible = True
        LoadJobsTMP()
    End Sub
    Private Sub LoadJobsTMP()
        ...
        End Try
    End Sub
    Private Sub BTN_FILTER_FCTM_Click(sender As Object, e As EventArgs) Handles BTN_FILTER_FCTM.Click
        Dim connectionString As String = DatabaseConnection.ConnectionString
        Dim query As String = "SELECT [ID], [Panel], [QTY], [ASGM Date] FROM [Jobs] WHERE [Panel Identifier] = 'Tee' AND [Status] IS NULL"
        Dim filters As New List(Of String)
        Dim parameters As New List(Of SqlParameter)

        If Chckd_lst_FCTM.CheckedItems.Count > 0 Then
            Dim JobFilter As New List(Of String)
            Dim index As Integer = 0
            For Each item As String In Chckd_lst_FCTM.CheckedItems
        ...
        Try
            Using connection As New SqlConnection(connectionString)
                ...
                Using reader As SqlDataReader = command.ExecuteReader()
                    DGV_FCTM_Panels.Rows.Clear() ' Limpiar filas existentes
                    While reader.Read()
                        Dim panelID As String = reader("ID").ToString()
                        Dim panel As String = reader("Panel").ToString()
                        Dim qty As Integer = Convert.ToInt32(reader("QTY"))
        ...
        Panel_FCTM.Visible = False
    End Sub
    Private Sub LNK_LBL_TM_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LNK_LBL_TM.LinkClicked
        Me.Hide()
