...
Public Class GuillotineM
    ...
    Private Sub Lnk_lbl_Tee_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles Lnk_lbl_Tee.LinkClicked
        ...
        Me.Close()
    End Sub
    Private Sub Btn_Save_Guillotine_Click(sender As Object, e As EventArgs) Handles Btn_Save_Guillotine.Click
        SavePanels()
    End Sub
    Private Sub SavePanels()
        Dim dgvList As List(Of DataGridView) = New List(Of DataGridView) From {
        DGV_Guillotine1, DGV_Guillotine_2
    }
    ...
    Private Sub UpdateCutDate()
        ...
        End Try
    End Sub
    Private Sub GuillotineM_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadGMPanels()

        DTP_GU.Value = DateTime.Now
        DGV_Guillotine1.AllowDrop = True
        DGV_Guillotine_2.AllowDrop = True
        Panel_Guillotine.Visible = False
    End Sub
    Private Sub LoadGMPanels()
        DGV_Panels_Guillotine.Rows.Clear()
                ...
                Using command As New SqlCommand(query, connection)
                    Using reader As SqlDataReader = command.ExecuteReader()
                        While reader.Read()
                            ...
                            DGV_Panels_Guillotine.Rows.Add(panelID, panel, qty, cutDate)
                        End While
                    End Using
                End Using

                If DGV_Panels_Guillotine.Columns.Count > 0 Then
                    DGV_Panels_Guillotine.Columns(0).HeaderText = "ID"
                    DGV_Panels_Guillotine.Columns(1).HeaderText = "Panel"
                    DGV_Panels_Guillotine.Columns(2).HeaderText = "Qty"
                    DGV_Panels_Guillotine.Columns(3).HeaderText = "Date"
                End If
            End Using
        ...
        End Try
    End Sub
    Private Function GetIDsFromDataGridView(dgv As DataGridView) As List(Of String)
        Dim ids As New List(Of String)()
        For Each row As DataGridViewRow In dgv.Rows
            If Not row.IsNewRow Then
                Dim id As String = Convert.ToString(row.Cells("ID").Value) ' Asegúrate de que la columna sea "ID"
                ids.Add(id)
            End If
        Next
        Return ids
    ...
    Private Sub DGV_Panels_Guillotine_DragDrop(sender As Object, e As DragEventArgs) Handles DGV_Panels_Guillotine.DragDrop
        If e.Data.GetDataPresent(DataFormats.Text) Then
            ...
            Dim rowToUpdate As DataGridViewRow = FindPanelRow(panel, DGV_Guillotine1, DGV_Guillotine_2)
            ...
            If rowToUpdate IsNot Nothing Then
                If originalQty > 1 Then
                    ...
                    If panelsQtyForm.ShowDialog() = DialogResult.OK Then
                        ...
                        newRow.CreateCells(DGV_Panels_Guillotine)
                        ...
                        DGV_Panels_Guillotine.Rows.Add(newRow)
                        ...
                        Else
                            If DGV_Guillotine1.Rows.Contains(rowToUpdate) Then
                                DGV_Guillotine1.Rows.Remove(rowToUpdate) ' Eliminar si la cantidad restante es 0
                            ElseIf DGV_Guillotine_2.Rows.Contains(rowToUpdate) Then
                                DGV_Guillotine_2.Rows.Remove(rowToUpdate) ' Eliminar si la cantidad restante es 0
                            End If
                ...
                Else
                    ...
                    DGV_Panels_Guillotine.Rows.Add(newRow)
                    RemoveRowFromSource(panel, DGV_Guillotine1, DGV_Guillotine_2)
                End If
            Else
    ...
    Private Function FindPanelRow(panel As String, ParamArray gridViews() As DataGridView) As DataGridViewRow
        ...
        Return Nothing
    End Function
    Private Sub DGV_Floors_MouseDown(sender As Object, e As MouseEventArgs) Handles DGV_Panels_Guillotine.MouseDown
        If e.Button = MouseButtons.Left Then
            Dim hitTestInfo As DataGridView.HitTestInfo = DGV_Panels_Guillotine.HitTest(e.X, e.Y)
            If hitTestInfo.RowIndex >= 0 Then
                Dim panel As String = DGV_Panels_Guillotine.Rows(hitTestInfo.RowIndex).Cells(1).Value.ToString()
                ' Iniciar el arrastre con el panel como dato
                DGV_Panels_Guillotine.DoDragDrop(panel, DragDropEffects.Move)
            End If
        End If
    End Sub
    Private Sub DGV_Guillotine1_DragEnter(sender As Object, e As DragEventArgs) Handles DGV_Guillotine1.DragEnter
        If e.Data.GetDataPresent(DataFormats.Text) Then
        ...
        End If
    End Sub
    Private Sub DGV_Guillotine1_DragDrop(sender As Object, e As DragEventArgs) Handles DGV_Guillotine1.DragDrop
        If e.Data.GetDataPresent(DataFormats.Text) Then
            Dim panel As String = CType(e.Data.GetData(DataFormats.Text), String)
            ...
            For Each row As DataGridViewRow In DGV_Panels_Guillotine.Rows
            ...
            If rowToUpdate IsNot Nothing Then
                ...
                Dim newRow As DataGridViewRow = New DataGridViewRow()
                newRow.CreateCells(DGV_Guillotine1)
                newRow.Cells(0).Value = rowToUpdate.Cells(0).Value
                newRow.Cells(1).Value = panel
                newRow.Cells(2).Value = selectedQty
                newRow.Cells(3).Value = DTP_GU.Value
                DGV_Guillotine1.Rows.Add(newRow)

                Dim remainingQty As Integer = originalQty - selectedQty
                If remainingQty > 0 Then
                    rowToUpdate.Cells(2).Value = remainingQty
                Else
                    DGV_Panels_Guillotine.Rows.Remove(rowToUpdate)
                End If
            Else
                MessageBox.Show("No se encontró el panel.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
        End If
    End Sub
    Private Sub DGV_Guillotine1_DragOver(sender As Object, e As DragEventArgs) Handles DGV_Guillotine1.DragOver
        e.Effect = DragDropEffects.Move
    End Sub
    Private Sub DGV_Guillotine1_KeyDown(sender As Object, e As KeyEventArgs) Handles DGV_Guillotine1.KeyDown
        If e.KeyCode = Keys.Delete Then
            HandlePanelDeletion(DGV_Guillotine1)
        End If
    End Sub
    Private Sub DGV_Guillotine1_MouseDown(sender As Object, e As MouseEventArgs) Handles DGV_Guillotine1.MouseDown
        Dim hit As DataGridView.HitTestInfo = DGV_Guillotine1.HitTest(e.X, e.Y)
        If hit.RowIndex >= 0 AndAlso hit.ColumnIndex >= 0 Then
            Dim cell As DataGridViewCell = DGV_Guillotine1.Rows(hit.RowIndex).Cells(hit.ColumnIndex)
            If cell.Selected Then
                Dim dragValue As String = DGV_Guillotine1.Rows(hit.RowIndex).Cells(0).Value.ToString()
                DGV_Guillotine1.DoDragDrop(dragValue, DragDropEffects.Move)
            End If
        End If
    End Sub
    Private Sub DGV_Guillotine_2_DragEnter(sender As Object, e As DragEventArgs) Handles DGV_Guillotine_2.DragEnter
        If e.Data.GetDataPresent(DataFormats.Text) Then
            e.Effect = DragDropEffects.Move
        Else
            e.Effect = DragDropEffects.None
        End If
    End Sub
    Private Sub DGV_Guillotine_2_DragDrop(sender As Object, e As DragEventArgs) Handles DGV_Guillotine_2.DragDrop
        If e.Data.GetDataPresent(DataFormats.Text) Then
            Dim panel As String = CType(e.Data.GetData(DataFormats.Text), String)
            ...
            If rowToUpdate IsNot Nothing Then
                ...
                Dim newRow As DataGridViewRow = New DataGridViewRow()
                newRow.CreateCells(DGV_Guillotine_2)
                newRow.Cells(0).Value = rowToUpdate.Cells(0).Value
                newRow.Cells(1).Value = panel
                newRow.Cells(2).Value = selectedQty
                newRow.Cells(3).Value = DTP_GU.Value
                DGV_Guillotine_2.Rows.Add(newRow)
        ...
        End If
    End Sub
    Private Sub DGV_Guillotine_2_MouseDown(sender As Object, e As MouseEventArgs) Handles DGV_Guillotine_2.MouseDown
        Dim hit As DataGridView.HitTestInfo = DGV_Guillotine_2.HitTest(e.X, e.Y)
        If hit.RowIndex >= 0 AndAlso hit.ColumnIndex >= 0 Then
            Dim cell As DataGridViewCell = DGV_Guillotine_2.Rows(hit.RowIndex).Cells(hit.ColumnIndex)
            If cell.Selected Then
                Dim dragValue As String = DGV_Guillotine_2.Rows(hit.RowIndex).Cells(0).Value.ToString()
                DGV_Guillotine_2.DoDragDrop(dragValue, DragDropEffects.Move)
            End If
        End If
    End Sub
    Private Sub DGV_Guillotine_2_KeyDown(sender As Object, e As KeyEventArgs) Handles DGV_Guillotine_2.KeyDown
        If e.KeyCode = Keys.Delete Then
            HandlePanelDeletion(DGV_Guillotine_2)
        End If
    End Sub
    Private Sub DGV_Guillotine_2_DragOver(sender As Object, e As DragEventArgs) Handles DGV_Guillotine_2.DragOver
        e.Effect = DragDropEffects.Move
    End Sub
    ...
    Private Sub MovePanelBackToDGVWalls(panelID As String, panel As String, qty As Integer)
        ...
        For Each wallRow As DataGridViewRow In DGV_Panels_Guillotine.Rows
        ...
        If Not existsInWalls Then
            Dim newRow As DataGridViewRow = CType(DGV_Panels_Guillotine.Rows(0).Clone(), DataGridViewRow)
    ...
    Private Sub Btn_Menu_Click(sender As Object, e As EventArgs) Handles Btn_Menu.Click
        ...
        Me.Close()
    End Sub
    Private Sub Clear_GM_Click(sender As Object, e As EventArgs) Handles Clear_GM.Click
        ...
        Next
    End Sub
    Private Sub Print_M1_GM_Click(sender As Object, e As EventArgs) Handles Print_M1_GM.Click
        SavePanels()
        Dim queryAB As String = "SELECT [ASGMT Date], [Panel], [Qty] FROM [Molds] WHERE [Mold] = 'DGV_Guillotine1' AND [Estatus] = 'NUEVO' ORDER BY [ASGMT Date] ASC"
        ...
        AddHandler printDoc.PrintPage, Sub(s, ev)
                                           ...
                                           Dim headerText As String = "Guillotine Molds"
    ...
    End Sub
    Private Sub Print_M2_GM_Click(sender As Object, e As EventArgs) Handles Print_M2_GM.Click
        SavePanels()
        Dim queryAB As String = "SELECT [ASGMT Date], [Panel], [Qty] FROM [Molds] WHERE [Mold] = 'DGV_Guillotine_2' AND [Estatus] = 'NUEVO' ORDER BY [ASGMT Date] ASC"
        Dim dtAB As New DataTable()
        ...
        AddHandler printDoc.PrintPage, Sub(s, ev)
                                           ...
                                           Dim headerFont As New Font("Arial", 20, FontStyle.Bold)
                                           Dim headerText As String = "Guillotine Mold"
                                           Dim headerWidth As Single = ev.Graphics.MeasureString(headerText, headerFont).Width
                                           Dim centerX As Single = (ev.PageBounds.Width - headerWidth) / 2
                                           ev.Graphics.DrawString(headerText, headerFont, Brushes.Black, centerX, yPos)
                                           yPos += headerFont.GetHeight(ev.Graphics) + 20
                                           ...
                                           ' Encabezados de las columnas
                                           ev.Graphics.DrawString("Date", columnHeaderFont, Brushes.Black, tableStartX, yPos)
                                           ev.Graphics.DrawString("Mold 2", columnHeaderFont, Brushes.Black, tableStartX + dateColumnWidth, yPos)
                                           ev.Graphics.DrawString("Qty", columnHeaderFont, Brushes.Black, tableStartX + dateColumnWidth + panelColumnWidth, yPos)
                                           yPos += lineHeight
        ...
        printPreviewDialog.ShowDialog()
    End Sub
    Private Sub Print_GM_Click(sender As Object, e As EventArgs) Handles Print_GM.Click
        SavePanels()

        Dim queryAB As String = "SELECT [ASGMT Date], [Mold], [Panel], [Qty] " &
                        "FROM [Molds] " &
                        "WHERE ([Mold] = 'DGV_Guillotine1' OR [Mold] = 'DGV_Guillotine_2'  " &
                        "AND [Estatus] = 'NUEVO' " &
        ...
        AddHandler printDoc.PrintPage, Sub(s, ev)
                                           ...
                                           Dim headerFont As New Font("Arial", 20, FontStyle.Bold)
                                           Dim headerText As String = "Guillotine"
                                           Dim headerWidth As Single = ev.Graphics.MeasureString(headerText, headerFont).Width
                                           Dim centerX As Single = (ev.PageBounds.Width - headerWidth) / 2
                                           ev.Graphics.DrawString(headerText, headerFont, Brushes.Black, centerX, yPos)
                                           yPos += headerFont.GetHeight(ev.Graphics) + 20
    ...
    Private Sub FilterPanelsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles FilterPanelsToolStripMenuItem.Click
        Panel_Guillotine.Visible = True
        LoadJobsGM()
    End Sub
    Private Sub LoadJobsGM()
        ...
        Try
            Using connection As New SqlConnection(connectionString)
                ...
                Dim query As String = $"SELECT DISTINCT [Job Number] FROM Jobs WHERE [Status] IS NULL"
                Using command As New SqlCommand(query, connection)
                    Using reader As SqlDataReader = command.ExecuteReader()
                        While reader.Read()
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        Dim connectionString As String = DatabaseConnection.ConnectionString
        Dim query As String = "SELECT [ID], [Panel], [QTY], [ASGM Date] FROM [Jobs] WHERE [Status] IS NULL"
        Dim filters As New List(Of String)
        Dim parameters As New List(Of SqlParameter)

        If Chckd_lst_GM.CheckedItems.Count > 0 Then
            Dim JobFilter As New List(Of String)
            Dim index As Integer = 0
            For Each item As String In Chckd_lst_GM.CheckedItems
        ...
        Try
            Using connection As New SqlConnection(connectionString)
                ...
                Using reader As SqlDataReader = command.ExecuteReader()
                    DGV_Panels_Guillotine.Rows.Clear() ' Limpiar filas existentes
                    While reader.Read()
                        Dim panelID As String = reader("ID").ToString()
                        Dim panel As String = reader("Panel").ToString()
                        Dim qty As Integer = Convert.ToInt32(reader("QTY"))
                        ...
                        DGV_Panels_Guillotine.Rows.Add(panelID, panel, qty, DateSGM)
                    End While
                End Using
            End Using
    ...
    Private Sub Txt_Bx_Jobs_TextChanged(sender As Object, e As EventArgs) Handles Txt_Bx_Jobs.TextChanged
        ...
        For i As Integer = 0 To Chckd_lst_GM.Items.Count - 1
            Dim itemText As String = Chckd_lst_GM.Items(i).ToString().ToLower()

            If itemText.Contains(searchJob) Then
                Chckd_lst_GM.SetItemChecked(i, True)
            Else
                Chckd_lst_GM.SetItemChecked(i, False)
            End If
        Next
    End Sub
